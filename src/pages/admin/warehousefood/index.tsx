...
const AdminWareHouse = () => {
  const { data: stock, refetch: refetchStock }: any =
    useGetStocksQuery<IStock[]>();
  ...
  const dataSource = stock?.data?.data
    ?.map((stock: IStock) => ({
      key: stock.id,
      ...stock,
    }))
  ...
  return (
    <div className="table table-borderless table-thead-bordered table-nowrap table-align-middle card-table  ">
      ...
      >
        <span>
          Bạn có chắc muốn thay đổi trạng thái thành{" "}
          <strong>
            {selectedStock?.stock_status === "Hết hàng"
              ? "Còn hàng"
              : "Hết hàng"}
            ?
          </strong>
